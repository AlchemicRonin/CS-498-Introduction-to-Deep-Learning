...
def train(D, G, D_solver, G_solver, discriminator_loss, generator_loss, show_every=250, batch_size=128, noise_size=100,
          num_epochs=10, train_loader=None, device=None):
    """
    Train loop for GAN.
    ...
    for epoch in range(num_epochs):
        ...
        for x, _ in train_loader:
            ...
            D_solver.zero_grad()
            z_space = sample_noise(batch_size, noise_size).reshape((batch_size, noise_size, 1, 1)).to(device)
            fake_images = G(z_space).reshape(batch_size, input_channels, img_size, img_size).to(device)
